...
async function loadData() {
  try {
    const qResponse = await fetch("questions.json", { cache: "no-store" });
...
function showRoundResult() {
  ...
  resultGif.className = "gif-container neon";
  resultGif.innerHTML = `<img src="${roundWon ? roundResults[0].winGif : roundResults[0].loseGif}" alt="Resultado da rodada">`;
...
function showGameResult() {
  ...
  finalGif.className = "gif-container neon";
...
function adjustBackground() {
  ...
  body.style.backgroundPosition = isMobile ? 'center top' : 'center center';
